...
- When a user pauses a goal: the goal is excluded from daily plan generation immediately; if this leaves the user with zero tasks (all goals paused), the daily plan shows an empty state with a prompt to resume a goal or create a new one; paused goals MUST NOT accumulate review backlogs — their `next_review_at` dates are not advanced while paused
- When a goal reaches 100% topic completion: the goal remains `active`; new-learning tasks for that goal stop appearing in the daily plan; only spaced review tasks continue; a one-time celebration state is shown on first reaching 100% (goal card and path timeline); the goal is NOT automatically archived — the user can manually archive it when reviews are no longer desired
...
- **FR-003**: System MUST generate a personalized daily study plan containing 2–5 prioritized tasks in the typical case, respecting the user's stated daily time availability and balancing new learning with scheduled reviews; the time budget constraint takes precedence over the 2-task minimum — when the budget only fits one task, the plan MUST show that single task rather than padding or returning an empty plan; the minimum is 1 task whenever at least one task fits within the time budget; when all topics in a goal's learning path are complete, that goal MUST contribute only spaced review tasks to the daily plan (no new-learning tasks); if no reviews are due for a fully-completed goal on a given day, that goal contributes zero tasks
...
- **FR-007**: System MUST provide a progress dashboard showing: study activity heatmap, topics completed per goal, current and longest learning streaks, and per-goal completion percentage; when a goal first reaches 100% topic completion, the system MUST display a one-time celebration state (e.g., trophy/confetti indicator) on the goal card and path timeline; subsequent visits show the completed state without the celebration animation
...
- **Learning Goal**: A defined knowledge outcome the user wants to achieve, with title, priority, target date, and status; status enum: `active` (contributes tasks to daily plan and reviews), `paused` (excluded from daily plans; review dates frozen until resumed), `archived` (permanently excluded; data retained for export only); transitions: `active ↔ paused`, `active/paused → archived`; archiving is irreversible
...
- Q: When a user pauses a goal, what happens to its tasks in the daily plan and scheduled reviews? → A: Suspend both — paused goals are excluded entirely from daily plan generation (no new-learning and no review tasks); spaced review dates freeze at their current scheduled date and resume unchanged on un-pause; archived goals are permanently excluded but data is retained
- Q: When all topics in a goal's path are 100% complete, what should happen? → A: Auto review-only mode — goal stays active; new-learning tasks stop; only scheduled spaced reviews surface in the daily plan; a one-time celebration state is shown on first reaching 100%; the goal is NOT auto-archived; user can manually archive when reviews are no longer desired
- Q: What level of production error observability should the app have? → A: Vercel logs only — rely on Vercel dashboard function logs for server-side error visibility; no external monitoring service (Sentry, etc.) required for v1
- Q: When available time only fits fewer than 2 tasks, what should the daily plan show? → A: Show whatever fits, minimum 1 task — time budget takes precedence over the 2-task floor; a single task is better than padding or an empty plan
- Q: When the user asks the AI coach about topics unrelated to their learning goals, how should the coach respond? → A: Answer freely, optionally connecting the response back to the user's goals when relevant — the coach is a general learning companion, not restricted to defined goal topics
...
- Production error observability is provided exclusively by Vercel's built-in function logs; no external error monitoring service (Sentry, Datadog, etc.) is required for v1; this is an explicit scope decision given the single-user, personal-tool nature of the product
